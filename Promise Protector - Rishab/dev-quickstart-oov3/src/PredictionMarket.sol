...
contract PredictionMarket is OptimisticOracleV3CallbackRecipientInterface {
    ...
    function initializeMarket(
        string memory "yes", // Short name of the first outcome.
        string memory "no", // Short name of the second outcome.
        string memory "if Canada meets its 2030 carbon goal then the winner recieves 0.1 GoerliETH if no then they lose 0.1 GoerliETH", // Description of the market.
        uint256 0.1, // Reward available for asserting true market outcome.
        uint256 0.05 // Expected bond to assert market outcome (OOv3 can require higher bond).
